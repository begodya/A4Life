...
//        objc_setAssociatedObject(presented, &key, self, UInt(objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC))
        return MenuTransitionAnimator(.Presentation)
    }

    func animationControllerForDismissedController(dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning? {
        dispatch_async(dispatch_get_main_queue()) {
//            objc_setAssociatedObject(dismissed, &key, nil, UInt(objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC))
        }
